...
make_thinknode_request_context(api_session& session)
{
    // TODO tasklet?
...
get_context_id(api_session& session, std::string realm)
{
    return cradle::get_context_id(
        make_thinknode_request_context(session), std::move(realm));
}
    ...
    bool ignore_upgrades)
{
    return cradle::get_iss_blob(
        make_thinknode_request_context(session),
        std::move(context_id),
        std::move(object_id),
        ignore_upgrades);
}
...
copy_iss_object(
    ...
    std::string object_id)
{
    thinknode_request_context trc{make_thinknode_request_context(session)};
    auto source_bucket
        = co_await cradle::get_context_bucket(trc, source_context_id);
